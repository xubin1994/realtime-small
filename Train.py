...
def main(args):
    ...
    with tf.name_scope('training_error'):
        ...
        full_reconstruction_loss = loss_factory.get_supervised_loss(args.lossType, multiScale=True, logs=False,
                                                                    weights=args.lossWeights, max_disp=MAX_DISP)(
            predictions, inputs)
    ...
    with tf.Session(config=tf.ConfigProto(gpu_options=gpu_options)) as sess:
        ...
        try:
            ...
            while True:
                tf_fetches = [global_step, train_op, full_reconstruction_loss, val_prediction2, val_prediction4,val_prediction]
...
if __name__ == '__main__':
	...
	parser.add_argument("-o", "--output", help="path to the output folder where the results will be saved",
						default='reslasnew/', type=str)
	parser.add_argument("--weights", help="path to the initial weights for the disparity estimation network (OPTIONAL)", default = 'reslas/')
