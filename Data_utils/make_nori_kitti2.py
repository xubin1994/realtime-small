...
nw = nori.open("/data/KITTI/nori2_KITTI_train.nori", "w")


nross = nori.Fetcher()
f = nori.utils.smart_open("s3://bucketjy/nori_KITTI_train.list", "r")
...
for data_id in lines:
    ...
    left_disp = Image.open('/unsullied/sharefs/_research_slam/data/KITTI_Depth_Pred_Eval/train/2011_10_03_drive_0042_sync/proj_depth/groundtruth/image_02/0000000005.png')
    left_disp = np.array(left_disp)
    #r['gt']  ###or left_disp = cv2.imdecode(np.frombuffer(r['gt'], dtype=np.uint16), cv2.IMREAD_GRAYSCALE)
