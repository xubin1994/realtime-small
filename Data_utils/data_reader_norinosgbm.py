...
def read_pickle(data_id):
    ...
    left_disp = r['gt']
    if left_disp.shape[-1]!=1:
        left_disp=np.expand_dims(left_disp, axis=-1)

    if 'sgbm' in r.keys():

        sgbm = r['sgbm']#  verified
        #sgbm = cv2.resize(sgbm, (left_img.shape[0], left_img.shape[1]), interpolation=cv2.INTER_NEAREST)反了！！
        # sgbm = cv2.resize(sgbm, (left_img.shape[1], left_img.shape[0]), interpolation=cv2.INTER_NEAREST) no need
...
class dataset():
    ...
    def _load_image(self, data_id):
        ...
        if self._usePfm:
            left_image, right_image, gt_image, sgbm_image, name = tf.py_func(lambda x: read_pickle(x), [data_id],
                                                                             [tf.uint8, tf.uint8, tf.float32,
                                                                              tf.float32, tf.string])  ##name自己消化
            gt_image.set_shape([None, None, 1])
            self._double_prec_las = False
        else:
            left_image, right_image, gt_image, sgbm_image, name = tf.py_func(lambda x: read_pickle(x), [data_id],
                                                                             [tf.uint8, tf.uint8, tf.int32,
                                                                              tf.float32, tf.string])  ##name自己消化
        ...
        if self._usePfm:##same as gt
            ...
            part1 = tf.zeros([270, tf.shape(line_image)[1], tf.shape(line_image)[2]])
            part2 = tf.zeros([tf.shape(line_image)[0] - 272, tf.shape(line_image)[1],
                              tf.shape(line_image)[2]])
            val = tf.cast(line_image[270:272], tf.float32)
            line_image = tf.concat([part1, val, part2], axis=0)
            #if self._double_prec_las:  #####不是即可
