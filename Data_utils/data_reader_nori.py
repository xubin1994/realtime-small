...
def read_pickle(data_id):
    ...
    sgbm = cv2.resize(sgbm, (left_img.shape[0], left_img.shape[1]), interpolation=cv2.INTER_NEAREST)
...
class dataset():
    ...
    def _load_image(self, data_id):
        ...
        left_image, right_image, gt_image, sgbm_image,name = tf.py_func(lambda x: read_pickle(x), [data_id], [tf.float32, tf.float32,tf.float32, tf.float32, tf.string])##name自己消化
        ...
        if sgbm_image != None:
            ...
            sgbm_image = tf.image.resize_image_with_crop_or_pad(sgbm_image)
    ...
    def _build_input_pipeline(self):
        ...
        # create dataset
        dataset = tf.data.Dataset.from_tensor_slices(self.pk_files).repeat(self._num_epochs)
        if self._shuffle:
