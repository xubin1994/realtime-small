...
def get_supervised_loss(name, multiScale=False, logs=False, weights=None, reduced=True, max_disp=None):
	...
	def compute_loss(disparities,inputs,conf_for_laser):
		...
		if multiScale:
			...
			disp_to_test=1
		valid_map = tf.where(tf.logical_and(tf.equal(targets, 0), tf.less(targets,max_disp)), tf.zeros_like(targets, dtype=tf.float32), tf.ones_like(targets, dtype=tf.float32))
		valid_map *= tf.where(tf.less_equal(targets,max_disp), tf.ones_like(targets, dtype=tf.float32), tf.zeros_like(targets, dtype=tf.float32))
...
def get_laser_loss(name, multiScale=False, logs=False, weights=None, reduced=True, max_disp=None):
	"""
	Build a lambda op to be used to compute a loss function
	Args:
		name: name of the loss function to build
	...
	def compute_loss(disparities, inputs,conf_for_laser=None):
		...
		if multiScale:
			...
			disp_to_test = 1
		valid_map = tf.where(tf.logical_and(tf.equal(targets, 0), tf.less_equal(targets, max_disp)),
							 tf.zeros_like(targets, dtype=tf.float32), tf.ones_like(targets, dtype=tf.float32))
