...
def main(args):
    ...
    with tf.name_scope('training_error'):
        ...
        #learning_rate = tf.train.exponential_decay(args.lr, global_step, args.decayStep, 0.5)# starcase = True
        learning_rate = tf.train.exponential_decay(args.lr, global_step, args.decayStep, 5e-4)
        disparity_trainer = tf.train.AdamOptimizer(learning_rate, 0.9)
...
if __name__ == '__main__':
    ...
    parser.add_argument("--trainingSet", help='path to the list file with training set',
                        default='s3://bucketjy/nori2_flying_train.list',
                        type=str)  ############
    ...
    parser.add_argument("--batchSize", help='batch size to use during training', type=int, default=4)#16
