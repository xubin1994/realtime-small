...
if split == "test_flying":

    for date in ['A','B','C']:
        root_dir = "/unsullied/sharefs/jiangying/data/flying/frames/frames_cleanpass/TEST/" + date
        laser1_dir = '/unsullied/sharefs/jiangying/data/flying/stereoSGBM/TEST/' + date
        laser0_dir = '/unsullied/sharefs/jiangying/data/flying/disparity/TEST/' + date
        dirs = os.listdir(laser0_dir)
        data_dirs = [a for a in os.listdir(root_dir) if a in dirs]
        for data_dir in data_dirs:
            left_dir = os.path.join(root_dir,data_dir, 'left')
            right_dir = os.path.join(root_dir, data_dir, 'right')
            laser_dir = os.path.join(laser0_dir,data_dir, 'left')
            laser11_dir = os.path.join(laser1_dir, data_dir, 'left')
            ...
            frames = [a for a in left_frame if (a in las_frame) and a in las11_frame]
            if len(frames) == 0:
                print(laser_dir)
                print(left_frame)
                print(las_frame)
            ...
            disp_L = sorted([os.path.join(laser_dir, fname + '.pfm') for fname in frames])
            las_L = sorted([os.path.join(laser11_dir, fname + '.png') for fname in frames])
...
if split == "test_laser_line":
    ...
    #dirs = os.listdir(laser0_dir)  "/unsullied/sharefs/jiangying/data/flying/frames/frames_cleanpass/**/*/***/left/****.png"

    for date in ['A','B','C']:
        root_dir = "/unsullied/sharefs/jiangying/data/flying/frames/frames_cleanpass/TRAIN/" + date
        laser0_dir = '/unsullied/sharefs/jiangying/data/flying/stereoSGBM/TRAIN/' + date
        dirs = os.listdir(laser0_dir)
        data_dirs = [a for a in os.listdir(root_dir) if a in dirs]#四位编号
        for data_dir in data_dirs:
            left_dir = os.path.join(root_dir,data_dir, 'left')
            right_dir = os.path.join(root_dir, data_dir, 'right')
            laser_dir = os.path.join(laser0_dir,data_dir, 'left')
...
elif split == 'train_flying':
    #laser0_dir = '/unsullied/sharefs/jiangying/data/Kitti_velo_origin/' + split
    #dirs = os.listdir(laser0_dir)
    for date in ['A','B','C']:
        root_dir = "/unsullied/sharefs/jiangying/data/flying/frames/frames_cleanpass/TRAIN/" + date
        laser1_dir = '/unsullied/sharefs/jiangying/data/flying/stereoSGBM/TRAIN/' + date
        laser0_dir = '/unsullied/sharefs/jiangying/data/flying/disparity/TRAIN/' + date
        dirs = os.listdir(laser0_dir)
        data_dirs = [a for a in os.listdir(root_dir) if a in dirs]
        for data_dir in data_dirs:
            left_dir = os.path.join(root_dir,data_dir, 'left')
