...
def main(args):
    ...
    with tf.name_scope('input_reader'):
        with tf.name_scope('training_set_reader'):
            ...
            left_img_batch, right_img_batch, gt_image_batch, laser_image_batch = data_set.get_batch()
            inputs = {
                'left': left_img_batch,
        ...
        if args.validationSet is not None:
            with tf.name_scope('validation_set_reader'):
                ...
                left_val_batch, right_val_batch, gt_val_batch, laser_val_batch = validation_set.get_batch()#print(left_val_batch.shape, right_val_batch.shape)# build network
    with tf.variable_scope('model') as scope:
        ...
        if args.validationSet is not None:
            ...
            val_prediction4 = val_stereo_net.get_disparities()[-3]
            val_prediction2 = val_stereo_net.get_disparities()[-5]
    ...
    if args.validationSet is not None:
        ...
        with tf.variable_scope('validation_error'):
            ...
            abs_err_ori = tf.abs(laser_val_batch - gt_val_batch)
            ...
            valid_map_ori = valid_map*tf.where(tf.equal(laser_val_batch, 0), tf.zeros_like(laser_val_batch, dtype=tf.float32),
            ...
            abs_err_ori = tf.reduce_sum(filtered_error_ori) / tf.reduce_sum(valid_map_ori)


            bad_5_perc = tf.where(tf.greater(tf.abs(abs_err), 0.05),
            ...
            bad_5_perc_ori = tf.where(tf.greater(tf.abs(abs_err_ori), 0.05),
            ...
            delta_err = tf.maximum(val_prediction/(gt_val_batch+1.0-valid_map),gt_val_batch/(val_prediction+1.0-valid_map))
            ...
            tf.summary.scalar('abs_err_ori', abs_err_ori)
            tf.summary.scalar('rmse', rm)
            tf.summary.scalar('bad3', bad_pixel_perc)
            tf.summary.scalar('bad3_ori', bad_pixel_perc_ori)
            ...
            tf.summary.image('val_prediction2', preprocessing.colorize_img(val_prediction2, cmap='jet'), max_outputs=1)
            tf.summary.image('val_prediction4', preprocessing.colorize_img(val_prediction4, cmap='jet'), max_outputs=1)
    ...
    with tf.name_scope('training_error'):
        ...
        full_reconstruction_loss = loss_factory.get_supervised_loss(args.lossType, multiScale=True, logs=False,
                                                                    weights=args.lossWeights, max_disp=MAX_DISP)(predictions, inputs)
    ...
    with tf.Session(config=tf.ConfigProto(gpu_options=gpu_options)) as sess:
        ...
        try:
            ...
            while True:
                tf_fetches = [global_step, zhuyi!!!!! full_reconstruction_loss, val_prediction2, val_prediction4,val_prediction]
                ...
                if step_eval % 100 == 0:
                    ...
                    print(
                        'Step:{:4d}\tLoss:{:.2f}\tf/b time:{:3f}\tMissing time:{}'.format(step_eval, fetches[1], fbTime,
                                                                                          datetime.timedelta(
                                                                                              seconds=missing_time)))
                    start_time = time.time()
                    #np.save('disparities.npy', fetches[-2])
...
if __name__ == '__main__':
    ...
    parser.add_argument("--trainingSet", help='path to the list file with training set', default='csvFile2test_laser_2012.csv',
                        type=str)  ############
    parser.add_argument("--validationSet", help="path to the list file with the validation set",
                        default='csvFile2train_laser_2012.csv', type=str)#
    parser.add_argument("-o", "--output", help="path to the output folder where the results will be saved",
                        default='reslas/', type=str)
